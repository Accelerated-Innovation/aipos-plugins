...
the category says. This is the same rule `aipos-feature-slice`'s rubric applies scenario by
...
Real sizing is `aipos-feature-slice`'s job, on real scenarios, with the Scenario Complexity Matrix. Do not attempt point scores here; a number invented before the scenarios exist is a number the team will quote back for months.
