...
4. **Run the verifier** (`python3 scripts/verify_canvas.py <canvas.json> --write`). Translate any
   error into one plain sentence — never read out a code. *"The −30% target and the 1.2-minute target
...
1. **Mode** — above.
2. **An existing canvas?** Look for `solution-design/<slug>/canvas.json` in the project folder. If
   one exists, go to [Resuming a canvas](#resuming-a-canvas).
3. **The graph** — detect it and choose the problem (`opportunity-source.md`). No graph → say so
...
7. **Where it's kept** — once: *"I'll keep the canvas in `solution-design/<slug>/` in this project
   as we go, so we can pick it up later. OK?"* Local files need one confirmation for the set; trackers
   are Step 10.
8. **Title and goal** — propose both from the graph's title, in outcome terms: *"Reduce triage time
...
2. **Pain points.** Propose 1–3 from what the graph establishes, each with its references.
...
5. **The hypothesis** — draft *If we …, then we can …, without …* from the answers; confirm it.
...
- **Impact at scale** — volume from the graph or a GAP; the unit conversion; the verifier computes
  the rest. Show the formula if an input is a GAP: *"0.6 minutes × GAP tickets ÷ 60 — it becomes a
...
6. Ask for approval of the content. On an explicit yes, set `stage: approved`. Approving the content
   is not approval to write anywhere outside the project folder.

## Step 9 — Render

```bash
python3 scripts/render_canvas.py solution-design/<slug>/canvas.json
```
...
**The project folder** (`solution-design/<slug>/`): `canvas.json`, the render, and `todo.md` when
there is no linked ticket. Never overwrite an approved canvas — a re-run writes
...
1. **Attachment** — neither connector uploads files. Hand it over by exact filename:
   *"Attach `canvas.pdf` to OPP-12 — I can't upload files myself."* Fallback: the canvas summary plus
   `canvas.json` as a second comment.
