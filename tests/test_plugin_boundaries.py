...
def test_shared_references_stay_canonical_in_the_plugin_that_owns_them():
    ...
    restates.
    """
    owned = {p.name for p in (skill_path("govkit-feature-map").parent.parent / "references").glob("*.md")}
    assert "spec-identifiers.md" in owned
    assert "workflow-source.md" in owned

    duplicates = [
        p for p in PLUGINS.rglob("references/*.md")
        if p.name in owned and p.parent != skill_path("govkit-feature-map").parent.parent / "references"
    ]
