...
[`aipos-feature-refine`](./plugins/govkit/skills/aipos-feature-refine/SKILL.md) runs that 3 Amigos review against a 10-dimension quality rubric: it finds blockers, names evidence gaps, and suggests rewritten Gherkin. It ends with a Development Token *recommendation* — a recommendation, because the gate itself lives in the repo, not in the tracker.
...
[`aipos-feature-readiness`](./plugins/govkit/skills/aipos-feature-readiness/SKILL.md) validates the actual package in your repository — `acceptance.feature`, `nfrs.md`, `eval_criteria.yaml` — against a 12-dimension gate, and issues the token. Not the tracker's opinion of readiness. The repo's.
...
[`aipos-feature-map`](./plugins/govkit/skills/aipos-feature-map/SKILL.md) ingests from Jira, Aha!, or a repo directory and renders one self-contained HTML page: a producer/consumer chain diagram, a card per feature with its full spec, readiness and size badges, and MVP/V1/V2 slice views. Run it on an epic and the weak link is usually visible in about four seconds.

[`aipos-workflow-map`](./plugins/govkit/skills/aipos-workflow-map/SKILL.md) answers the question a feature list cannot: how does a customer actually get from start to finish, and who does what along the way. It authors `workflow.json` — the single source the L1/L2/L3 views are generated from — referencing the Gherkin that already exists rather than restating it, and reports the behavior in your corpus that no step in the journey touches.

[`aipos-metrics-emit`](./plugins/govkit/skills/aipos-metrics-emit/SKILL.md) emits Tier 1 metric events as NDJSON from the repo's own exhaust — spec completeness, gate readiness, delivery inputs. And [`aipos-synthetic-data`](./plugins/govkit/skills/aipos-synthetic-data/SKILL.md) closes the loop with seeded, repeatable Faker generators derived from the feature's own Gherkin scenarios, so the tests an agent writes have data that matches the spec it was given.
...
- **[aipos-quarterly-planning](./plugins/aipos-p1/skills/aipos-quarterly-planning/SKILL.md)**: The quarterly session pack in five moves — portfolio read, evidence grading (Strong / Thin / Stale / Say-so, the grade beside the rank), gap analysis (coverage, attention, expiry pressure, funnel signals), the one-page agenda with its capacity ledger, and the Exploration Decision log. Every claim provenance-marked and traceable to an MCP tool call; the skill never writes — decisions enter the system of record through its own governed doors. Includes a dry-run rehearsal mode.
...
- **[aipos-epic-create](./plugins/govkit/skills/aipos-epic-create/SKILL.md)**: A problem-first interview that produces a complete epic — a problem statement with no solution smuggled into it, named personas, quantified impact, OKR alignment, three-to-five measurable success metrics, dated evidence, MVP scope with a real out-of-scope list, risks, and NFRs. In GenAI mode it sets the evaluation criteria every feature under the epic inherits. Where a Pillar 2 viability brief exists, this is the skill that writes out the handoff inputs it named.
- **[aipos-feature-create](./plugins/govkit/skills/aipos-feature-create/SKILL.md)**: Break an epic into a workflow-aligned, non-overlapping feature set via lightweight story mapping, then author one feature into Draft 0 — user stories, structured description, auto-tagged Gherkin, NFRs, Definition of Done, privacy impact. Writes a repo feature package by default; creates or updates Jira and Aha! records only after an explicit, destination-named confirmation. Switches on evaluation-driven authoring automatically for GenAI features.

**Spec and gate**

- **[aipos-feature-refine](./plugins/govkit/skills/aipos-feature-refine/SKILL.md)**: The 3 Amigos review of a generated Draft 0. Scores against a 10-dimension quality rubric, finds blockers and evidence gaps, suggests rewritten Gherkin, and produces a Development Token recommendation. Also exposes a non-interactive batch mode for scoring a whole corpus.
- **[aipos-feature-readiness](./plugins/govkit/skills/aipos-feature-readiness/SKILL.md)**: The repo-side 12-dimension gate that actually issues the Development Token, once the package (`acceptance.feature`, `nfrs.md`, `eval_criteria.yaml`) is in the repository.

**Sizing and planning**

- **[aipos-feature-slice](./plugins/govkit/skills/aipos-feature-slice/SKILL.md)**: Scenario sizing on the Scenario Complexity Matrix and MoSCoW release slicing onto `@mvp` / `@v1` / `@v2` tags. Proposes splits for Large scenarios, flags Large scenarios on the MVP critical path, and can write the tagged spec back to Jira or Aha! after an explicit confirmation.
- **[aipos-workflow-map](./plugins/govkit/skills/aipos-workflow-map/SKILL.md)**: Authors the `workflow.json` behind a customer journey — outcome, ordered and branching activities, actors, the handoffs where work changes hands, and qualified references into canonical Gherkin. L1 journey, L2 collaboration and L3 behavior are generated from that one file, so a Rule shown at four steps is authored once and nobody maintains a diagram. Reports behavior in the corpus that no step touches.
- **[aipos-feature-map](./plugins/govkit/skills/aipos-feature-map/SKILL.md)**: The cross-cutting corpus view — a self-contained HTML page with a producer/consumer chain diagram, one card per feature with its full spec, readiness badges, and size badges with MVP/V1/V2 slice views. Ingests from Jira, Aha!, or a repo directory, and merges tracker records with repo-resident Gherkin.

**Execution support**

- **[aipos-synthetic-data](./plugins/govkit/skills/aipos-synthetic-data/SKILL.md)**: A seeded, repeatable Python Faker generator derived from the feature's Gherkin scenarios, plus committed data files.
- **[aipos-metrics-emit](./plugins/govkit/skills/aipos-metrics-emit/SKILL.md)**: Structured Tier 1 metric events (NDJSON) from a governed repo's exhaust — spec completeness, gate readiness, velocity and quality inputs. The producer side of the AIPOS federated metrics topology.
