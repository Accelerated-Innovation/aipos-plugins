...
>
> [GovKit](./plugins/aipos/README.md)
...
[`aipos-feature-refine`](./plugins/aipos/skills/aipos-feature-refine/SKILL.md) runs that 3 Amigos review against a 10-dimension quality rubric: it finds blockers, names evidence gaps, and suggests rewritten Gherkin. It ends with a Development Token *recommendation* — a recommendation, because the gate itself lives in the repo, not in the tracker.
...
[`aipos-feature-readiness`](./plugins/aipos/skills/aipos-feature-readiness/SKILL.md) validates the actual package in your repository — `acceptance.feature`, `nfrs.md`, `eval_criteria.yaml` — against a 12-dimension gate, and issues the token. Not the tracker's opinion of readiness. The repo's.
...
[`aipos-feature-map`](./plugins/aipos/skills/aipos-feature-map/SKILL.md) ingests from Jira, Aha!, or a repo directory and renders one self-contained HTML page: a producer/consumer chain diagram, a card per feature with its full spec, readiness and size badges, and MVP/V1/V2 slice views. Run it on an epic and the weak link is usually visible in about four seconds.

[`aipos-workflow-map`](./plugins/aipos/skills/aipos-workflow-map/SKILL.md) answers the question a feature list cannot: how does a customer actually get from start to finish, and who does what along the way. It authors `workflow.json` — the single source the L1/L2/L3 views are generated from — referencing the Gherkin that already exists rather than restating it, and reports the behavior in your corpus that no step in the journey touches.

[`aipos-metrics-emit`](./plugins/aipos/skills/aipos-metrics-emit/SKILL.md) emits Tier 1 metric events as NDJSON from the repo's own exhaust — spec completeness, gate readiness, delivery inputs. And [`aipos-synthetic-data`](./plugins/aipos/skills/aipos-synthetic-data/SKILL.md) closes the loop with seeded, repeatable Faker generators derived from the feature's own Gherkin scenarios, so the tests an agent writes have data that matches the spec it was given.
...
- **[aipos-quarterly-planning](./plugins/aipos/skills/aipos-quarterly-planning/SKILL.md)**: The quarterly session pack in five moves — portfolio read, evidence grading (Strong / Thin / Stale / Say-so, the grade beside the rank), gap analysis (coverage, attention, expiry pressure, funnel signals), the one-page agenda with its capacity ledger, and the Exploration Decision log. Every claim provenance-marked and traceable to an MCP tool call; the skill never writes — decisions enter the system of record through its own governed doors. Includes a dry-run rehearsal mode.

Plugin: [`aipos-p1`](./plugins/aipos/README.md) · `/plugin install aipos-p1@aipos`
...
- **[aipos-epic-create](./plugins/aipos/skills/aipos-epic-create/SKILL.md)**: A problem-first interview that produces a complete epic — a problem statement with no solution smuggled into it, named personas, quantified impact, OKR alignment, three-to-five measurable success metrics, dated evidence, MVP scope with a real out-of-scope list, risks, and NFRs. In GenAI mode it sets the evaluation criteria every feature under the epic inherits. Where a Pillar 2 viability brief exists, this is the skill that writes out the handoff inputs it named.
- **[aipos-feature-create](./plugins/aipos/skills/aipos-feature-create/SKILL.md)**: Break an epic into a workflow-aligned, non-overlapping feature set via lightweight story mapping, then author one feature into Draft 0 — user stories, structured description, auto-tagged Gherkin, NFRs, Definition of Done, privacy impact. Writes a repo feature package by default; creates or updates Jira and Aha! records only after an explicit, destination-named confirmation. Switches on evaluation-driven authoring automatically for GenAI features.

**Spec and gate**

- **[aipos-feature-refine](./plugins/aipos/skills/aipos-feature-refine/SKILL.md)**: The 3 Amigos review of a generated Draft 0. Scores against a 10-dimension quality rubric, finds blockers and evidence gaps, suggests rewritten Gherkin, and produces a Development Token recommendation. Also exposes a non-interactive batch mode for scoring a whole corpus.
- **[aipos-feature-readiness](./plugins/aipos/skills/aipos-feature-readiness/SKILL.md)**: The repo-side 12-dimension gate that actually issues the Development Token, once the package (`acceptance.feature`, `nfrs.md`, `eval_criteria.yaml`) is in the repository.

**Sizing and planning**

- **[aipos-feature-slice](./plugins/aipos/skills/aipos-feature-slice/SKILL.md)**: Scenario sizing on the Scenario Complexity Matrix and MoSCoW release slicing onto `@mvp` / `@v1` / `@v2` tags. Proposes splits for Large scenarios, flags Large scenarios on the MVP critical path, and can write the tagged spec back to Jira or Aha! after an explicit confirmation.
- **[aipos-workflow-map](./plugins/aipos/skills/aipos-workflow-map/SKILL.md)**: Authors the `workflow.json` behind a customer journey — outcome, ordered and branching activities, actors, the handoffs where work changes hands, and qualified references into canonical Gherkin. L1 journey, L2 collaboration and L3 behavior are generated from that one file, so a Rule shown at four steps is authored once and nobody maintains a diagram. Reports behavior in the corpus that no step touches.
- **[aipos-feature-map](./plugins/aipos/skills/aipos-feature-map/SKILL.md)**: The cross-cutting corpus view — a self-contained HTML page with a producer/consumer chain diagram, one card per feature with its full spec, readiness badges, and size badges with MVP/V1/V2 slice views. Ingests from Jira, Aha!, or a repo directory, and merges tracker records with repo-resident Gherkin.

**Execution support**

- **[aipos-synthetic-data](./plugins/aipos/skills/aipos-synthetic-data/SKILL.md)**: A seeded, repeatable Python Faker generator derived from the feature's Gherkin scenarios, plus committed data files.
- **[aipos-metrics-emit](./plugins/aipos/skills/aipos-metrics-emit/SKILL.md)**: Structured Tier 1 metric events (NDJSON) from a governed repo's exhaust — spec completeness, gate readiness, velocity and quality inputs. The producer side of the AIPOS federated metrics topology.

Plugin: [`govkit`](./plugins/aipos/README.md) · `/plugin install govkit@aipos`
...
|---|---|
| `aipos-feature-readiness/scripts/readiness_state.py` | A Development Token becomes a **derived execution-readiness record referencing the product approval**, not a substitute for one. Five states stay apart — prepared, approved for exact scope, locally executable, authority verified, implementation verified — and a stale reading cannot produce an authoritative green. |
| `aipos-feature-refine/scripts/change_package.py` | Proposals against approved behavior become a **reapproval request that cannot approve anything**: no field can express a decision, nothing claims to have been submitted, and a request that would not be decidable is refused rather than written. |
| `aipos-feature-map/scripts/render_map.py` | Cards **name the blocked commitment**, keeping a blocked commitment distinct from a Blocked token — different failures with different fixes. The chip is a snapshot; the gate runs in CI. |
...
- **Pillar 2 refuses to write specs.** Ask `aipos-rapid-validation` for Gherkin or NFRs and you get a polite redirect — specs are born *at* the Validation Decision, not before it.
...
- [`plugins/aipos/references/gherkin-authoring-standard.md`](./plugins/aipos/references/gherkin-authoring-standard.md) — the shared Gherkin authoring standard: BRIEF, explicit `Rule:` blocks, boundary coverage, scenario isolation, the confirmed/proposed/unresolved registers, what makes a spec automatable without turning it into implementation instructions, and the line between a deterministic behavior check and an aggregate GenAI evaluation.
- [`plugins/aipos/references/spec-identifiers.md`](./plugins/aipos/references/spec-identifiers.md) — `@rule:` / `@scenario:` identifiers that survive rewording, connecting rules, scenarios, NFRs, evaluations and evidence. Optional and additive; existing packages keep working.
- [`plugins/aipos/references/workflow-source.md`](./plugins/aipos/references/workflow-source.md) — the `workflow.json` format: a customer outcome, ordered and branching activities, actors and handoffs, and qualified references into canonical Gherkin. L1 journey / L2 collaboration / L3 behavior are views of one file, and the diagram is a generated projection — a Rule shown at four steps is authored once.
